...
import { borrarInvitado as deleteGuestByToken, guardarInvitado, guardarInvitados, obtenerInvitados, } from "../services/invitadosService";
...
export async function loadGuestsAdminData() {
...
export async function createGuest(invitados, draft) {
    if (!draft.nombre.trim() || !draft.grupo.trim())
        return null;
    const nextId = invitados.length > 0
        ? String(Math.max(...invitados.map((guest) => {
...
export async function removeGuest(token, nombre) {
...
export async function assignGuestToTable(invitados, mesas, token, mesaId) {
    const invitadosActualizados = invitados.map((guest) => guest.token === token
        ? (() => {
            const updated = { ...guest };
            if (mesaId) {
