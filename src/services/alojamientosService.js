...
export async function obtenerAlojamientos() {
    if (!supabaseConfig.enabled) {
        return readStorageWithSchema(LODGING_OPTIONS_KEY, lodgingOptionsSchema, []);
    }
    return [];
}
export async function guardarAlojamientos(lista) {
    if (!supabaseConfig.enabled) {
...
export async function borrarAlojamiento(id) {
    if (!supabaseConfig.enabled) {
        const lista = readStorageWithSchema(LODGING_OPTIONS_KEY, lodgingOptionsSchema, []);
...
export async function obtenerSolicitudesAlojamiento() {
    if (!supabaseConfig.enabled) {
        return readStorageWithSchema(LODGING_REQUESTS_KEY, lodgingRequestsSchema, []);
    }
    return [];
}
export async function obtenerSolicitudAlojamientoPorInvitado(guestToken) {
    const requests = await obtenerSolicitudesAlojamiento();
    return requests.find((request) => request.guestToken === guestToken) ?? null;
}
export async function guardarSolicitudAlojamiento(request) {
    const requests = await obtenerSolicitudesAlojamiento();
